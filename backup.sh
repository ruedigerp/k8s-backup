...
LOG_FILE="/tmp/pv-backup-$(date +%Y%m%d-%H%M%S).log"
...
create_backup_job() {
    ...
    log "Erstelle Backup-Job für PVC $pvc_name"
...
spec:
  ...
  nfs:
    server: 10.0.10.7
    path: /srv/nfs/k8s-pv/production/k8s-backup
  claimRef:
    namespace: $namespace
    name: $job_name-pvc
---
...
EOF
    ...
    log "Warte bis PVC $job_name-pvc gebunden ist..."
...
EOF
    ...
    log "Warte auf Abschluss des Backup-Jobs..."
    ...
    # Optional: Backup-PVC nach erfolgreichem Backup aufräumen
    # Kommentiere die nächsten zwei Zeilen aus, wenn Backups dauerhaft gespeichert werden sollen
    # kubectl delete pvc/$job_name-pvc -n "$namespace"
    # kubectl delete pv/$job_name-pv
    
    log "Backup für PVC $pvc_name abgeschlossen"
}

# Hauptfunktion
main() {
    log "Starte PV-Backup-Prozess"
    
    # Prüfen ob kubectl verfügbar ist
    if ! command -v kubectl &> /dev/null; then
        log "FEHLER: kubectl ist nicht installiert oder nicht im PATH"
        exit 1
    fi
    ...
    log "Erstelle globale RBAC-Ressourcen..."
    cat <<EOF | kubectl apply -f -
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRole
metadata:
...
EOF
    ...
    # Alle PVs abrufen
    log "Sammle alle Persistent Volumes..."
    local pvs=$(kubectl get pv -o jsonpath='{.items[*].metadata.name}')
    
    if [[ -z "$pvs" ]]; then
        log "Keine Persistent Volumes gefunden"
        exit 0
    fi
    
    local pv_count=$(echo "$pvs" | wc -w)
    log "Gefunden: $pv_count Persistent Volumes"
    ...
    for pv in $pvs; do
        current=$((current + 1))
        log "===== Verarbeite PV $current/$pv_count: $pv ====="
        ...
        if [[ "$backup_labels" == "k8s-backupnfs" ]]; then
            log "PV $pv ist ein Backup-Volume - überspringe"
            continue
        fi
        ...
        if [[ -z "$pvc_info" || "$pvc_info" == " " ]]; then
            log "PV $pv hat keine PVC-Bindung - überspringe"
            continue
        fi
        ...
        if [[ "$pvc_name" =~ ^backup-.*-pvc$ ]]; then
            log "PVC $pvc_name ist ein Backup-PVC - überspringe"
            continue
        fi
        
        log "PVC: $pvc_name in Namespace: $pvc_namespace"
        
        # Prüfen ob PVC existiert
        if ! kubectl get pvc "$pvc_name" -n "$pvc_namespace" &>/dev/null; then
            log "PVC $pvc_name existiert nicht in Namespace $pvc_namespace - überspringe"
            continue
        fi
        ...
        log "PVC $pvc_name hat Access Modes: $access_modes"
        
        if [[ -z "$pods" ]]; then
            log "Keine Pods nutzen PVC $pvc_name - erstelle direktes Backup"
            create_backup_job "$pvc_name" "$pvc_namespace"
            continue
        fi
        
        log "Pods die PVC $pvc_name nutzen: $(echo "$pods" | tr '\n' ' ')"
        
        # Prüfen ob ReadWriteMany - dann können Pods weiterlaufen
        if [[ "$access_modes" == *"ReadWriteMany"* ]]; then
            log "PVC $pvc_name hat ReadWriteMany Access Mode - Pods können weiterlaufen"
            create_backup_job "$pvc_name" "$pvc_namespace"
            continue
        fi
        
        log "PVC $pvc_name hat ReadWriteOnce/ReadOnlyMany - Pods müssen gestoppt werden"
        ...
        for pod in $pods; do
            ...
            if [[ "$controller" != "none" ]]; then
                ...
                if ! grep -q "^$controller$" "$controllers_file" 2>/dev/null; then
                    ...
                    log "Controller gefunden: $controller mit $current_replicas Replicas"
                fi
            fi
        done
        ...
        log "Backup für PV $pv abgeschlossen"
    done
    
    log "===== Alle PV-Backups abgeschlossen ====="
    log "Logfile: $LOG_FILE"
    
    # Optional: Cleanup aller temporären ClusterRoleBindings
    log "Cleanup: Entferne temporäre ClusterRoleBindings..."
